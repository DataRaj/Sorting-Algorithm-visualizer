...
function createNewArray(noOfBars = 50) {
    ...
    for (let i = 0; i < noOfBars; i++) {
        const fulbar = document.createElement('div')
        const bar = document.createElement("div");
        const barVal = document.createElement("span")
        fulbar.className = 'fulbar'
        barVal.className = 'value'
        barVal.innerHTML = barArray[i];
        // console.log(barVal.innerHTML)   
        bar.style.height = `${barArray[i]*2}px`;
        bar.classList.add('bar');
        bar.classList.add('flex-item');
        bar.classList.add(`barNo${i}`);
        fulbar.appendChild(barVal)
        fulbar.appendChild(bar)
        bars.appendChild(fulbar);
    }
}
...
const randomize = document.querySelector(".new");
